#!/usr/bin/env python3
"""gbsync — hub-and-spoke, pull-on-demand sync dispatcher for gsd-beads.
...
never sync to each other. Two directions:
  ...
  PULL  (tools -> bd)   reconcile-on-demand
        ...
        both-sides-changed cases are written to .gsd-beads/conflicts.json.

State files (all under <project>/.gsd-beads/):
...
def main():
    ...
    base = Path(project_dir) / ".gsd-beads"
    cfg = load_json(base / "sync.json", None)
    if cfg is None:
        die(f"no {base/'sync.json'} — run /gsd-beads:sync-config first")
