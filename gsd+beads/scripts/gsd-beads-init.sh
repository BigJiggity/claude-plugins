#!/usr/bin/env bash
# gsd-beads-init — bootstrap a project for the GSD<->beads integration.
# Ensures git + beads are live so the gsd-beads skill/hook activates.
# GSD's own .planning/ is created interactively by /gsd:new-project inside Claude.
#
# Usage:  gsd-beads-init.sh [target-dir]   (defaults to current dir)
set -euo pipefail
...
▸ next steps (inside Claude Code):
  ...
  The gsd-beads skill activates once both .planning/ and .beads/ exist.
NEXT
