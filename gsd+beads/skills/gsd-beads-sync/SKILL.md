...
- **both changed since the last pull → conflict**: recorded in
  `.gsd-beads/conflicts.json` (with the LWW resolution). Review that file and
  fix by hand if the auto-resolution was wrong.

After a pull, **review `.gsd-beads/conflicts.json`** and surface any entries to
the user.

## State files (under `.gsd-beads/`)
