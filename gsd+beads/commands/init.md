...
Bootstrap the current working directory so the `gsd-beads` integration
activates. Do the following:

1. Run the bootstrap script:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/gsd-beads-init.sh" "$PWD"
   ```
...
3. Tell the user the remaining steps must run inside Claude:
   - `/gsd:new-project` to create `.planning/` and the ROADMAP. After the
     roadmap exists, follow the `gsd-beads` skill: create one bd issue per
     requirement, label each `phase-N`, and write each `NN-BEADS-MAP.md`.
   - Then `/gsd:plan-phase 1`, `/gsd:execute-phase 1`, etc.

4. Note: the `gsd-beads` skill keys off both `.planning/` and `.beads/` being
