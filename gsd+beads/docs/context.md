# gsd-beads context — intent-aware memory for context-mode

Complete guide to the context-mode integration in the `gsd-beads` plugin: what
...
`gsd-beads context` gives that memory **intent**. It ties context-mode's
...
gsd+beads/
├── skills/
│   └── gsd-beads-context/SKILL.md   # the convention + lifecycle + boundaries
├── commands/
│   └── context-config.md            # /gsd-beads:context-config (opt-in)
├── hooks/
...
| Piece | Role |
|---|---|
| `gsd-beads-context` skill | The rules: label on index, filter on search, switch on phase, guard on stats. Activates when opted in and `ctx_*` tools exist. |
| `session-start.sh` | Injects a one-screen reminder of the convention — **only** when `.gsd-beads/context.json` is present. |
...
source = gb/<bd_id>/<phase>
         ...
         └─────────────── fixed prefix, namespaces all gsd-beads memory
```
...
| All gsd-beads memory | `ctx_search(source: "gb/")` | everything this layer indexed |
...
already tracked by the `gsd-beads` skill. No extra state file.
...
# 2. Opt in — interactive; writes .gsd-beads/context.json:
/gsd-beads:context-config

# 3. That's it. The gsd-beads-context skill activates automatically now that the
...
All state lives under `<project>/.gsd-beads/`:
...
Run these alongside the `gsd-beads` lifecycle (claim → in_progress → close):

| gsd-beads / GSD step | context action |
|---|---|
...
|---|---|
| Skill never engages | `.gsd-beads/context.json` missing → run `/gsd-beads:context-config`; or the `ctx_*` tools aren't loaded (context-mode not installed). |
| No SessionStart reminder | The hook only emits when `.gsd-beads/context.json` exists. Create it via the command. |
...
- Config in `.gsd-beads/context.json` (committed, no secrets); presence = opt-in.
