...
Complete guide to the two-way synchronization layer in the `gsd-beads` plugin:
...
`gsd-beads sync` mirrors your [beads](https://github.com/gastownhall/beads)
...
1. Read `.gsd-beads/sync.json`; collect enabled backends.
...
**Conflicts** are written to `.gsd-beads/conflicts.json` with which side won;
...
/gsd-beads:sync-config        # interactive; writes .gsd-beads/sync.json

# 3. Export the API tokens it tells you to (see §7). Then:
#    - PUSH happens automatically during the gsd-beads lifecycle, or manually:
...
| Command | Does |
|---|---|
| `/gsd-beads:init` | Bootstrap a repo: `git init` (if needed) + `bd init`. |
| `/gsd-beads:sync-config` | Interactively choose backends and write `sync.json`; tells you which env vars to export. |
| `/gsd-beads:sync-pull` | Reconcile external edits back into bd; summarizes conflicts. |
...
When the repo also uses GSD + bd (the `gsd-beads` skill), fire the matching push
...
Run `/gsd-beads:sync-pull` whenever someone may have edited issues in an
...
|---|---|
| `no .gsd-beads/sync.json` | Run `/gsd-beads:sync-config` first. |
...
- Config in `.gsd-beads/sync.json` (committed, env-var names only).
